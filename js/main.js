...
$( document ).ready(function() {
    ...
    // Attach a stopsort handler for .sortable() event to update the db
    // with the new weight and possibly category for each task
    ...
    // Attach a Sortable handle to #categories to allow cards to be
    // sorted using the fa-arrows-alt icon and update the db with new order
    var el = document.getElementById('categories');
    var sortable = new Sortable(el, {
      handle: ".fa-arrows-alt",
      filter: ".form-group",
      draggable: ".card",
      onEnd: function (evt) { // When a move is completed
        sort = {};
...
function updateBucketElements(bucket) {
  ...
  $('#bucketActions').html('<i id="addNewCategory" title="Add Category"  data-bucket-id="'+bucket['bucket_id'] +'" data-toggle="modal" data-target="#addCategory" class="far fa-plus-square fa-2x"></i>'
    + '<i id="editBucket" title="Edit Bucket" data-bucket-id="'+bucket['bucket_id'] +'" data-toggle="modal" data-target="#addBucket" data-type="bucket" class="far fa-edit fa-2x" ></i>'
...
function createCategoryCard(category) {
  ...
  var card = '<div class="card bg-light mb-3" id="'+category_id+'" data-category-id='+category['category_id']+'>'
            ...
            +'    </div>'
            +'  </div>'
            +'  <ul class="list-group list-group-flush" id="'+category_id+'-ul" data-category-id='+category['category_id']+'>'
            +'  </ul>'
            +' </div>'
            +'</div>';
